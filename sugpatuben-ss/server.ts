...
// Re-encode a VFR video to constant frame rate, reporting progress (0-100)
async function convertToCfr(
  inPath: string,
  outPath: string,
  targetFps: number,
  durationSec: number,
  ...
  try {
    const proc = new Deno.Command(FFMPEG, {
      args: [
        "-y", "-i", inPath,
        "-fps_mode", "cfr", "-r", String(targetFps),
    ...
    while (true) {
      ...
      for (const line of lines) {
        // -progress emits key=value lines; out_time=HH:MM:SS.micros
        const m = line.match(/^out_time=(\d+):(\d+):(\d+(?:\.\d+)?)/);
        if (m && durationSec > 0) {
          const t = Number(m[1]) * 3600 + Number(m[2]) * 60 + Number(m[3]);
          onProgress(Math.min(100, Math.round((t / durationSec) * 100)));
        }
      }
    }
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        controller.close();
          ...
          // Video: if the file has variable frame rate, convert to constant
          if (mode === "video") {
            const probe = await probeFrameRate(expectedFile);
            if (probe?.vfr) {
              send("progress", {
                percent: 96,
                status: `Variabel bildfrekvens upptäckt – konverterar till konstant (${probe.targetFps} fps)...`,
              });
              const cfrFile = join(DOWNLOADS_DIR, `${id}.cfr.mp4`);
              const ok = await convertToCfr(
                expectedFile,
                cfrFile,
                probe.targetFps,
                probe.durationSec,
                (pct) => {
                  // Map conversion 0-100% into the 96-99% band
                  send("progress", {
                    percent: 96 + Math.min(3, Math.floor(pct / 34)),
                    status: `Konverterar till konstant bildfrekvens: ${pct}%`,
                  });
                },
              );
              if (ok) {
                await Deno.rename(cfrFile, expectedFile);
              } else {
                // Conversion failed — serve the original rather than nothing
                try { await Deno.remove(cfrFile); } catch { /* ignore */ }
                send("progress", { percent: 99, status: "Konvertering misslyckades – original behålls" });
              }
            }
          }
          ...
          send("done", { file: `${id}.${ext}`, filename });
        } catch (e) {
